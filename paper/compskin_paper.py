...
Figure 1: Starting from a random initialization of skinning weights, our method converges to a linear blend skinning approximation of input blendshapes. The accuracy and visual quality of our results is comparable to the state-of-the-art (Dem Bones),
but introduces significantly smaller run-time overheads. In the figure, each color visualizes the influence of one bone, with 40
bones total.
ABSTRACT
We present a new method to bake classical facial animation blendshapes into a fast linear blend skinning representation. Previous
work explored skinning decomposition methods that approximate
general animated meshes using a dense set of bone transformations; these optimizers typically alternate between optimizing for
the bone transformations and the skinning weights. We depart from
this alternating scheme and propose a new approach based on proximal algorithms, which effectively means adding a projection step
...
Ladislav Kavan, John Doublestein, Martin Prazak, Matthew Cioffi, and Doug
Roble. 2024. Compressed Skinning for Facial Blendshapes. In Special Interest Group on Computer Graphics and Interactive Techniques Conference
Conference Papers ’24 (SIGGRAPH Conference Papers ’24), July 27-August 1,
2024, Denver, CO, USA. ACM, New York, NY, USA, 9 pages. https://doi.org/
10.1145/3641519.3657477
1 INTRODUCTION
...
fast and integrates well with a GPU that performs the final rendering. Linear blend skinning decomposition methods [James and
Twigg 2005] have been proposed to solve this problem for general
...
data matrix A ∈ R3𝑆×𝑁 as a product of two factors: A ≈ BC, where
B ∈ R3𝑆×4𝑃, C ∈ R4𝑃 ×𝑁 . The input matrix A contains the x, y,
z coordinates of the 𝑆 input shapes with 𝑁 vertices; the 𝑃 is the
number of proxy-bones. The matrices B and C have special structure: B is typically dense and stacks the skinning transformations;
...
the user has to select a fixed number of proxy-bones. These proxybones do not have any anatomical meaning, but they correspond
to individual skinning transformations; each transformation drives
part of the mesh. With our rigs, we found that as few as 40 proxybones lead to acceptable quality with Dem Bones. However, even
...
between optimization for the skinning weights and the transformations (cyclic coordinate descent). More and more efficient and
general solvers were explored during the 2005 - 2015 decade and
open sourced in 2019 in the Dem Bones library. The need to minimize the run-time compute requirements lead us to explore an
...
not trivial, because typical deep learning optimizers assume unconstrained optimization and typically handle bounds via sigmoid
...
constraint-projection step to the popular Adam optimizer [Kingma
and Ba 2014]; this is a key ingredient of our proposed method.
...
However, this objective is highly non-convex and thus different optimization methods converge to different solutions. Our approach
...
is that we can introduce sparsity constraints also on the transformations, i.e., zero-out about 90% of the coefficients of B and still
match the accuracy of the Dem Bone results (this is in addition
to the sparsity of C, i.e., both B and C are now sparse). This additional sparsification is straightforward to implement as part of
the constraint-projection step. Even when accounting for the overhead of sparse data structures, this represents significant savings at
runtime. To our knowledge, this additional “skinning transformation sparsification” has not been explored before and we propose
...
compact implementation in PyTorch [Paszke et al. 2019] with automatic backward differentiation and trivial deployment to CUDA.
...
from the classical 𝐿2 norm to an 𝐿𝑝 norm with higher 𝑝. When we
changed the norm to 𝐿12 and increased the number of influences to
32, the maximal error dropped by more than 50 times compared to
Dem Bones, allowing us to recover even fine wrinkles in the original blendshapes, at the cost of more non-zeros weights in matrix C.
However, our main focus is not improvements of quality but rather
run-time efficiency.
Another contribution is a precise formulation of converting the
“blend-weights” (i.e. the time-varying blendshape coefficients) to
linear blend skinning transformations. This problem was not considered in previous skinning decomposition methods [James and
Twigg 2005; Le and Deng 2012] which were more general and not
focused on blendshape animation. This conversion is not hard but
also it is not trivial because 1) skinning handles the rest pose differently than blendshapes and 2) linear blending of transformations is
correct only when using a carefully chosen subset of spatial transformations (specifically, we use linearized rotations). Our first-order
...
Our main technical contributions are:Compressed Skinning for Facial Blendshapes SIGGRAPH Conference Papers ’24, July 27-August 1, 2024, Denver, CO, USA
• New, first-order optimization method that solves skinning
decomposition better than previous methods
• New skinning decomposition constraint: sparsity of transformations (“compressed skinning”)
• Explicit formulas to convert facial animation blend-weights
into skinning transformations
However, our method is more complex than classical linear blend
skinning and drifts away from established practices, in particular:
• Our method needs an extra sparse matrix-vector multiplication on the CPU to compute skinning transformations.
• The GPU skinning shaders need to support arbitrary transformation matrices (rigid transformations are not sufficient).
...
In this section we briefly recap standard methods for facial blendshape modeling and establish notation for future sections. The
...
𝑆∑︁𝑘=1
𝑐𝑘 (vˆ𝑘,𝑖 − vˆ0,𝑖) (1)
where vˆ0,𝑖 ∈ R3 is 𝑖-th rest-pose vertex, vˆ𝑘,𝑖 ∈ R3 is 𝑖-th vertex
in blendshape number 𝑘 and 𝑐𝑘 are blending coefficients. With all
coefficients 𝑐𝑘 set to zero, we obtain the rest-pose, typically representing a neutral facial expression. In a basic linear blendshape
model, all 𝑐𝑘 are directly driven by the animator. However, the wellknown limitations of linear blending lead to dissatisfying results for
some combinations of 𝑐𝑘 values. To avoid this problem, artists typically do not modify the 𝑐𝑘 coefficients directly, but create a function
known as a “rig” that takes as input a reduced, animator-friendly
set of control values and outputs all of the 𝑐𝑘 coefficients. These
controls are typically FACS-based [Ekman and Friesen 1978] and
low-dimensional (about 40 to 80 controls), while the 𝑐𝑘 coefficients
...
deformations of articulated meshes [Magnenat-Thalmann et al.
1988], but it is a general deformation model [Jacobson et al. 2011]:
𝑃∑︁𝑗=1
𝑤𝑖,𝑗 M𝑗 v0,𝑖 (2)
where v0,𝑖 ∈ R4 is vˆ0,𝑖 but with the additional coordinate set to 1;
M𝑗 ∈ R3×4 are affine transformation matrices, corresponding to
either skeletal bones in full-body animation or virtual proxy-bones
[James and Twigg 2005]; 𝑤𝑖,𝑗 are skinning weights satisfying the
following constraints; non-negativity: 𝑤𝑖,𝑗 ≥ 0, partition of unity:
˝𝑗 𝑤𝑖,𝑗 = 1 and spatial sparsity: the number of nonzero 𝑤𝑖,𝑗 for
each 𝑖 is bounded by a constant 𝐾. Eq. 2 transforms only vertex
positions; normals are often approximated but an accurate and
efficient method exists [Tarini et al. 2014].SIGGRAPH Conference Papers ’24, July 27-August 1, 2024, Denver, CO, USA Ladislav Kavan, John Doublestein, Martin Prazak, Matthew Cioffi, and Doug Roble
The transformations M𝑗 are the only time-varying parameters
in an animation and thus play the same role as the 𝑐𝑘 coefficients
in blendshape models (Eq. 1). To convert a blendshape model to
a skinned one, we will need a way to convert 𝑐𝑘 into M𝑗. To our
...
3 FRAMEWORK
We start by writing down the formula for converting 𝑐𝑘 into M𝑗.
...
algorithm (Sec. 4).
We introduced the problem of skinning decomposition via a succinct abstraction using matrices A ∈ R3𝑆×𝑁 , B ∈ R3𝑆×4𝑃 and
C ∈ R4𝑃 ×𝑁 . The matrix A contains vˆ𝑘,𝑖 − vˆ0,𝑖 and the matrices
B, C have the following structure:
B =

Nˆ 1,1 . . . Nˆ 1,𝑃
. . . . . . . . .
...
(3)
The matrices Nˆ
𝑘,𝑗 ∈ R3×4 are defined as Nˆ 𝑘,𝑗 = I + N𝑘,𝑗, where
I ∈ R3×4 is a 3 × 3 identity matrix extended with a column of zeros.
With this notation, the formula BC = A is equivalent to:
𝑃∑︁𝑗=1
𝑤𝑖,𝑗 (I + N𝑘,𝑗 )v0,𝑖 = vˆ𝑘,𝑖 (4)
Due to the fact that ˝𝑗 𝑤𝑖,𝑗Iv0,𝑖 = vˆ0,𝑖 which follows from the
partition of unity of the skinning weights, Eq. 4 is also equivalent
to:
𝑃∑︁𝑗=1
𝑤𝑖,𝑗N𝑘,𝑗v0,𝑖 = vˆ𝑘,𝑖 − vˆ0,𝑖 (5)
which shows that the transformations N𝑘,1, . . . , N𝑘,𝑃 represent exactly the 𝑘-th delta-shape vˆ𝑘,𝑖 − vˆ0,𝑖 (Eq. 1). Intuitively, the subtraction of the identities I corresponds to subtraction of the rest
pose. This is related to the fact that linear blend skinning returns
the rest-pose for identity transformations, while Eq. 1 returns the
rest-pose for zero blendweights 𝑐𝑘.
Let us now assume that we have solved for 𝑤𝑖,𝑗 and N𝑘,𝑗 that
satisfy Eq. 5 and all of the skinning weight constraints (this will be
discussed in more detail in Sec. 4). Now we just need a formula to
compute the final skinning transformations. This formula can be
derived by algebraic manipulations of Eq. 1:
vˆ0,𝑖 +
𝑆∑︁𝑘=1
𝑐𝑘 (vˆ𝑘,𝑖 − vˆ0,𝑖) = vˆ0,𝑖 +
𝑆∑︁𝑘=1
𝑐𝑘
𝑃∑︁𝑗=1
𝑤𝑖,𝑗N𝑘,𝑗v0,𝑖
...
𝑆∑︁𝑘=1
𝑐𝑘N𝑘,𝑗 (7)
for 𝑗 = 1, . . . , 𝑃, the linear blend skinning formula (Eq. 2) produces
the exact same result as the blending of the blendshapes, as shown
in Eq. 6.
...
need to evaluate Eq. 7 on the CPU. In the case of dense N𝑘,𝑗, as
with Dem Bones implementation, this is a dense matrix-vector
product. We propose sparse N𝑘,𝑗 in which case this becomes a sparse
matrix-vector product. In either case, the resulting M𝑗 are dense
...
accomplished by multiplying our M𝑗 with the head transformation
...
our final N𝑘,𝑗 transformations will be zero, enabling us to realize
significant savings compared to dense methods such as Dem Bones.
3.1 Transformation representation
The most general choice for N𝑘,𝑗 are general R3×4 matrices, but
some authors proposed restricting the 3 × 3 components to rotations [Le and Deng 2012]. This works well for general animation
sequences which are not driven by delta-blending, but our approach
relies on the linear blending in Eq. 7. Even if we constrained N𝑘,𝑗
to rotations, the resulting M𝑗 will not be rotations due to Eq. 7.
...
(𝑟𝑘,𝑗,:) [Goldstein et al. 2002] and another three for the translation
(𝑡𝑘,𝑗,:). This class of transformations is closed under linear blending,
...
a projection/normalization step and safeguards to ensure shortestpath interpolation [Buss and Fillmore 2001]. In our case, linear
...
skinning which also lends itself to a well structured code.Compressed Skinning for Facial Blendshapes SIGGRAPH Conference Papers ’24, July 27-August 1, 2024, Denver, CO, USA
Pre-processing
rest pose 𝒗0,𝑖
blendshapes 𝒗𝑘,𝑖
...
𝑤𝑖,𝑗, 𝒗0,𝑖
CPU
Figure 2: The skinning decomposition is pre-computed offline (left). On the end-user device, we first load the pre-computed
𝑤𝑖,𝑗 and N𝑘,𝑗. Then, for each animation frame (runtime, right), we obtain c𝑘 from the rig and compute M𝑗. The skinning
transformations M𝑗 along with the rest-pose v0,𝑖 and weights 𝑤𝑖,𝑗 are passed to linear blend skinning module running on the
GPU.
4 COMPRESSED SKINNING DECOMPOSITION
In this section we discuss the details of our skinning decomposition (Figure 2: pre-processing). This is a non-convex optimization
problem:
min
𝑤𝑖,𝑗,N𝑘,𝑗
𝑁∑︁𝑖=1
𝑆∑︁𝑘=1
(𝐸𝑖,𝑘)𝑝, 𝐸𝑖,𝑘 = |v𝑘,𝑖 − v0,𝑖 − ∑︁
𝑗
𝑤𝑖,𝑗N𝑘,𝑗v0,𝑖 | (9)
where by default we use 𝑝 = 2 corresponding to the standard
...
of the skinning decomposition are the following constraints imposed on 𝑤𝑖,𝑗 : non-negativity, partition of unity and spatial sparsity
(Sec. 2.2). The spatial sparsity ensures the sparsity of C, which is a
classical feature of skinning decomposition [James and Twigg 2005].
On the other hand, the sparsification of B is a new contribution in
...
unity constraints are easy: for each vertex 𝑖, we simply normalize
the weights vector 𝑤𝑖,: which ensures that ˝𝑗 𝑤𝑖,𝑗 = 1. The nonnegativity and spatial sparsity constraints can be both satisfied by
a single projection (proximal operator for an indicator function):
for each weight vector 𝑤𝑖,:, we keep only the 𝐾 largest weights
...
via torch.no_grad).
With 𝑝 = 2, Eq. 9 is equivalent to the optimization problem solved
...
constraints also on the B matrix (the sparsity of C is standard in all
...
zeroing-out all but the largest 𝐿 elements of B in absolute value.
...
C-sparsity case is that with B, we can distribute the non-zeros
arbitrarily in the B matrix, whereas the C limits the non-zeros
to 𝐾 per column, due to the limitations of GPUs and standard
...
typically set 𝐿 to 6000, which corresponds to 1000 transformations
...
The flexibility of the PyTorch implementation invites experimentation with different values of 𝑝 in Eq. 9, corresponding to different
norms. A particularly interesting case is 𝑝 = ∞, in which case Eq. 9
minimizes the maximal deviation from the ground truth blendshapes. We call this the “HD” (High Detail) setting because the
...
𝐿𝑝 norm with a high 𝑝; experimentally, we found that 𝑝 = 12 works
well if the model has enough capacity in terms of the number of
non-zeros in B, C. However, in our primary goal of reducing the
compute overheads by maximizing the sparsity of B, C, we found
that higher 𝑝 can produce non-smooth results; therefore, we use
𝑝 = 2 by default.SIGGRAPH Conference Papers ’24, July 27-August 1, 2024, Denver, CO, USA Ladislav Kavan, John Doublestein, Martin Prazak, Matthew Cioffi, and Doug Roble
Original Our Method Dem Bones
Figure 3: Our method leads to results of acceptable visual
quality on various rigs and facial expressions, with errors
comparable to Dem Bones (red color corresponds to error of
5mm or more). However, our method enables more efficient
run-time.
Aura Jupiter
Proteus Bowen
Figure 4: Histograms of the errors of our method (dark blue)
and Dem Bones (light blue) in centimeters. Our method
achieves lower errors despite sparse skining transformations.
5 RESULTS
...
error (MXE). The MAE is the mean of 𝐸𝑖,𝑘 (Eq. 9) and tells us what
error can we expect for a randomly chosen vertex and blendshape.
The MXE = max𝑖,𝑘 𝐸𝑖,𝑘 measures the worst error. These metrics
correspond to 𝐿1 and 𝐿∞ norms. Note that in our task, we assume
...
errors as outliers.
In our first set of experiments, we set the total number of proxybones to 40 for both Dem Bones and our method, but we limit our
method to no more than 6000 non-zeros in the matrix B. This means
...
(Table 1). We set the number of influences 𝐾 to 8 for both Dem
...
𝛽1 = 0.9, 𝛽2 = 0.9; the whole optimization takes several minutes
on a single A6000 GPU. Dem Bones runs in under a minute on the
CPU. The pre-processing times are relatively unimportant compared to the reduction of the run-time overhead of blending skinning transformations (Eq. 7). This formula can be expressed as
matrix multiplication of the coefficients 𝑐𝑘 with the matrix B. With
our method, the B is sparse, but with Dem Bones and other previous methods, the matrix B is dense. Sparse matrix data structures
...
the run-time performance measured on Snapdragon 652, which isCompressed Skinning for Facial Blendshapes SIGGRAPH Conference Papers ’24, July 27-August 1, 2024, Denver, CO, USA
Table 1: Statistics of our testing rigs and achieved fitting accuracy for our method and Dem Bones. Both the maximum (MXE)
and mean (MAE) errors are in millimeters, with human-sized head models.
...
is only about 8𝑚𝑠 and this needs to accommodate everything, including full-body animation, rendering, background and typically
also multiple characters.
Table 2: Memory requirements for sparse (our method) and
dense transformations (Dem Bones), using 40 bones in both
cases.
...
Bowen 87k 486k 5.6×
Table 3: Run-time speed measurements for sparse (our
method) and dense transformations (Dem Bones), using 40
bones in both cases. We also report the rig evaluation time.
...
to decrease the number of bones 𝑃 in Dem Bones. We tried this
with 𝑃 = 20, 10, 5 and even 1 (i.e., single transformation for the
...
MXE increased from 0.565 (𝑃 = 40) to 0.89 (𝑃 = 20) and the MAE
...
similar sparsity as our method; specifically we have set the translation threshold as 1mm and the rotation threshold as 1 degree. These
...
Figure 5: Decreasing the number of bones in Dem Bones from
40 to 20 increases the error significantly.
In our “High-Detail” experiment, we tried to minimize MXE
by setting 𝑝 = 12 in Eq. 9, using 200 proxy-bones, disabling the
sparsification of our transformations and setting 𝐾 to 32. We used
...
show that our 𝑝 = 12 minimization achieved more than 50× lower
...
We have also implemented our method in Unity and benchmarked the runtimes on a modern Windows PC (AMD 3975WX
and NVIDIA RTX 3080). To stress-test the system, we are displaying 10 copies of each of our 4 characters (Figure 7). Since both the
...
(Table 6). Our method is 1.4 times faster than Dem Bones as wellSIGGRAPH Conference Papers ’24, July 27-August 1, 2024, Denver, CO, USA Ladislav Kavan, John Doublestein, Martin Prazak, Matthew Cioffi, and Doug Roble
as Unity’s native implementation of blendshapes. This implementation is optimized for cache coherency etc., but requires 4.2 times
longer GPU compute.
Original Our Method Dem Bones
Figure 6: “Proteus HD” experiment: our method with 57400
transforms and 𝐿12 norm captures finer detail than Dem
Bones with the same number of transformations.
Figure 7: Screenshot of our Unity app.
Table 4: Dem Bones results with lower numbers of bones 𝑃
(MXE / MAE in millimeters).
Model 𝑃 = 20 𝑃 = 10 𝑃 = 5 𝑃 = 1
...
Table 5: Errors with sparsified Dem Bones transformations
(40 bones) in millimeters.
...
6 CONCLUSION
We have presented a novel method for linear blend skinning decomposition and its integration into a facial animation pipeline. Our
new optimization strategy inspired by proximal algorithms outperforms the state-of-the-art and allows us to significantly reduce the
...
due to the first-order optimization approach that requires many iterations. Another limitation is that we consider the rig function as a
...
ACKNOWLEDGMENTS
We thank Brian Budge, Roman Fedotov, Ryan Goldade, Stephane
Grabli, Philipp Herholz, Petr Kadlecek, Binh Le, J.P. Lewis, Ronald
Mallet, Olga Sorkine and Yuting Ye for inspiring discussions and
the anonymous reviewers for constructive feedback.
REFERENCES
Michal Aharon, Michael Elad, and Alfred Bruckstein. 2006. K-SVD: An algorithm for
designing overcomplete dictionaries for sparse representation. IEEE Transactions
on signal processing 54, 11 (2006), 4311–4322.
Stephen W. Bailey, Dalton Omens, Paul Dilorenzo, and James F. O’Brien. 2020. Fast
and Deep Facial Deformations. ACM Transactions on Graphics 39, 4 (Aug. 2020),Compressed Skinning for Facial Blendshapes SIGGRAPH Conference Papers ’24, July 27-August 1, 2024, Denver, CO, USA
94:1–15. https://doi.org/10.1145/3386569.3392397 Presented at SIGGRAPH 2020,
Washington D.C..
Stephen W Bailey, Dave Otte, Paul Dilorenzo, and James F O’Brien. 2018. Fast and deep
deformation approximations. ACM Transactions on Graphics (TOG) 37, 4 (2018),
1–12.
Gaurav Bharaj, Thorsten Thormählen, Hans-Peter Seidel, and Christian Theobalt. 2012.
Automatically rigging multi-component characters. In Computer Graphics Forum,
Vol. 31. 755–764.
Samuel R Buss and Jay P Fillmore. 2001. Spherical averages and applications to spherical
splines and interpolation. ACM Transactions on Graphics (TOG) 20, 2 (2001), 95–126.
Prashanth Chandran, Gaspard Zoss, Markus Gross, Paulo Gotardo, and Derek Bradley.
2022. Facial Animation with Disentangled Identity and Motion using Transformers.
In Computer Graphics Forum, Vol. 41. Wiley Online Library, 267–277.
Byungkuk Choi, Haekwang Eom, Benjamin Mouscadet, Stephen Cullingford, Kurt Ma,
Stefanie Gassel, Suzi Kim, Andrew Moffat, Millicent Maier, Marco Revelant, et al.
2022. Animatomy: An animator-centric, anatomically inspired system for 3d facial
modeling, animation and transfer. In SIGGRAPH Asia 2022 Conference Papers. 1–9.
Timothy Costigan, Anton Gerdelan, Emma Carrigan, and Rachel McDonnell. 2016.
Improving blendshape performance for crowds with GPU and GPGPU techniques.
In Proceedings of the 9th International Conference on Motion in Games. 73–78.
David L Donoho. 2006. Compressed sensing. IEEE Transactions on information theory
52, 4 (2006), 1289–1306.
...
Nils Hasler, Thorsten Thormählen, Bodo Rosenhahn, and Hans-Peter Seidel. 2010.
Learning skeletons for shape and pose. In Proceedings of the 2010 ACM SIGGRAPH
symposium on Interactive 3D Graphics and Games. 23–30.
...
Ladislav Kavan, Rachel McDonnell, Simon Dobbyn, Jiří Žára, and Carol O’Sullivan.
2007. Skinning arbitrary deformations. In Proceedings of the 2007 symposium on
Interactive 3D graphics and games. 53–60.
Ladislav Kavan, P-P Sloan, and Carol O’Sullivan. 2010. Fast and efficient skinning
of animated meshes. In Computer Graphics Forum, Vol. 29. Wiley Online Library,
327–336.
Joonho Kim and Karan Singh. 2021. Optimizing UI layouts for deformable face-rig
manipulation. ACM Transactions on Graphics (TOG) 40, 4 (2021), 1–12.
Diederik P Kingma and Jimmy Ba. 2014. Adam: A method for stochastic optimization.
arXiv preprint arXiv:1412.6980 (2014).
Binh Huy Le and Zhigang Deng. 2012. Smooth Skinning Decomposition with Rigid
Bones. ACM Trans. Graph. 31, 6 (2012).
Binh Huy Le and Zhigang Deng. 2013. Two-layer sparse compression of dense-weight
blend skinning. ACM Transactions on Graphics (TOG) 32, 4 (2013), 1–10.
Binh Huy Le and Zhigang Deng. 2014. Robust and accurate skeletal rigging from mesh
sequences. ACM Transactions on Graphics (TOG) 33, 4 (2014), 1–10.
Daniel D Lee and H Sebastian Seung. 1999. Learning the parts of objects by nonnegative matrix factorization. Nature 401, 6755 (1999), 788–791.
John P Lewis, Ken Anjyo, Taehyun Rhee, Mengjie Zhang, Frederic H Pighin, and
Zhigang Deng. 2014. Practice and theory of blendshape facial models. Eurographics
(State of the Art Reports) 1, 8 (2014), 2.
...
3130813
Nadia Magnenat-Thalmann, Richard Laperrire, and Daniel Thalmann. 1988. Jointdependent local deformations for hand animation and object grasping. In In Proceedings on Graphics Interface 1988.
Peter Melchior, Rémy Joseph, and Fred Moolekamp. 2019. Proximal Adam: robust adaptive update scheme for constrained optimization. arXiv preprint arXiv:1910.10094
(2019).
Mark Meyer and John Anderson. 2007. Key point subspace acceleration and soft
caching. In ACM SIGGRAPH 2007 papers. 74–es.
Anastasia Moutafidou, Vasileios Toulatzis, and Ioannis Fudos. 2023. Deep fusible
skinning of animation sequences. The Visual Computer (2023), 1–21.
...
Transactions on Graphics (TOG) 32, 6 (2013), 1–10.
Neal Parikh, Stephen Boyd, et al. 2014. Proximal algorithms. Foundations and trends®
in Optimization 1, 3 (2014), 127–239.
Adam Paszke, Sam Gross, Francisco Massa, Adam Lerer, James Bradbury, Gregory
Chanan, Trevor Killeen, Zeming Lin, Natalia Gimelshein, Luca Antiga, et al. 2019.
Pytorch: An imperative style, high-performance deep learning library. Advances in
neural information processing systems 32 (2019).
Sarah Radzihovsky, Fernando de Goes, and Mark Meyer. 2020. Facebaker: Baking
character facial rigs with machine learning. In Special Interest Group on Computer
Graphics and Interactive Techniques Conference Talks. 1–2.
Scott Schaefer and Can Yuksel. 2007. Example-based skeleton extraction. In Symposium
on Geometry Processing. 153–162.
Jaewoo Seo, Geoffrey Irving, John P Lewis, and Junyong Noh. 2011. Compression and
direct manipulation of complex blendshape models. ACM Transactions on Graphics
(TOG) 30, 6 (2011), 1–10.
Marco Tarini, Daniele Panozzo, and Olga Sorkine-Hornung. 2014. Accurate and
efficient lighting for skinned models. In Computer Graphics Forum, Vol. 33. Wiley
Online Library, 421–428.
Robert Tibshirani. 1996. Regression shrinkage and selection via the lasso. Journal of
the Royal Statistical Society Series B: Statistical Methodology 58, 1 (1996), 267–288."""